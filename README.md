...
字符画 3D 桌面宠物 + 键鼠/屏幕时间统计工具。一个旋转的 3D 甜甜圈（或地球仪）蹲在你的任务栏旁，你每敲一下键盘、点一下鼠标，它都会转起来——同时默默记录你的每一次输入。
...
| 模块 | 说明 |
|------|------|
| 🍩 3D 字符画宠物 | donut.c 经典环面数学渲染，深彩虹渐变色环随旋转流动；支持甜甜圈/地球仪两种形状，投影缩放按最大姿态标定、旋转时尺寸恒定（[说明](./docs/形状填充与窗口尺寸.md)） |
