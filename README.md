...
| 🎨 右键菜单 | 纯右键交互：形状切换、尺寸调节、配色（含多套自定义外观）、字符集、字符密度、光晕、背景、随机转向、动效（含点击抖动）、音效反馈、不透明度、置顶/吸附开关、立即重新吸附、统计入口、退出 |
| 🎬 输入反馈动效 | 键盘/鼠标/滚轮三类输入脉冲反馈；四档动效时长（关闭/短/中/长）；点击左右抖动；每 1000 键击触发里程碑庆祝 |
| 🔊 音效反馈 | 程序化合成的「宇宙幽灵」音效：敲键/点击/滚轮各自触发随机幽灵音符（Web Audio 合成 + 反馈回声，无需音频文件） |
...
│       ├── types.ts         # 共享类型定义
│       ├── palette.ts       # 面板配色常量
│       ├── ghostAudio.ts    # 程序化合成的「宇宙幽灵」音效引擎（Web Audio）
